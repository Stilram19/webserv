...
/*   Updated: 2023/07/20 20:57:45 by obednaou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Location::set_directory_listing(const std::string &input)
{
	if (input != "ON" && input != "OFF")
		throw bad_input("directory listing is either 'ON' or 'OFF' !");
...
{
	if (access(input.c_str(), F_OK)) // check read permission
		throw bad_input("Invalid redirection pathname!");
...
void Location::set_root_path(const std::string &input)
{
	if (access(input.c_str(), F_OK))
		throw bad_input("Invalid root pathname!");
	_root_path = input;
}
...
{
	if (access(input.c_str(), F_OK))
		throw bad_input("Invalid index pathname!");
	_index_path = input;
}
...
{
	if (access(input.c_str(), F_OK))
		throw bad_input("Invalid upload pathname!");
	_upload_path = input;
}
...
void Location::set_cgi_handler(const std::string &input)
{
	...
	start = skip_blank(input, end);
	cgi_interpret = input.substr(start);
	if (access(cgi_interpret, F_OK))
		throw bad_input("Invalid cgi interpreter!");
...
void Location::set_allowed_http_method(const std::string &input)
{
	...
	while (input[start])
	{
		...
		if (!is_http_method(method))
			throw bad_input("Unkown http Method! (GET, POST, DELETE are the only supported methods).");
		_allowed_http_methods.push_back(method);
		start = skip_spaces(input, end);
	}
}
